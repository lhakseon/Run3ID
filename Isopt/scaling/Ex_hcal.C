...
void Ex_hcal(){
 ...
 TH2F *isoPptS = new TH2F("isoPptS","HCAL Isolation vs Pt",250,0,1000,1000,0,100);
  ...
  for(int i  = 0; i < t_S->GetEntries();i++){
    ...
    if(ppt < 200) continue;
    if(iso_Hcal == 0) continue;
    isoPptS->Fill(ppt,iso_Hcal);
  }
  ...
  if (outfile.is_open()) {
      double p1 = fn2->GetParameter(1);
      double p2 = fn2->GetParameter(2);
      outfile << "double hcal_scale1 = " << p1 << ";\n";
      outfile << "double hcal_scale2 = " << p2 << ";\n";
      outfile.close();
  ...
  IsoptScaling->GetYaxis()->SetTitle("HCAL Isolation");
  ...
  his2->GetYaxis()->SetTitle("HCAL Isolation");
  ...
  IsoptScaling2->GetYaxis()->SetTitle("HCAL Isolation");
  ...
  c4->SaveAs("Quadratic_Hcal.png");
  c4->SaveAs("Quadratic_Hcal.C");
  ...
  IsoptScalingLin->GetYaxis()->SetTitle("HCAL Isolation ");

  c6->cd(2);
  his2->Draw("colz");
  his2->GetXaxis()->SetTitle("Photon Pt GeVc^{-1}");
  his2->GetYaxis()->SetTitle("HCAL Isolation");
//  c6->SaveAs("Straightline.png");
//  c6->SaveAs("Straightline.C");
